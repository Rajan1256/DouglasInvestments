    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.users.update', [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('known_as') ? 'is-invalid' : '' }}" type="text" name="known_as" id="known_as" value="{{ old('known_as', $user->known_as) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sa_id') ? 'is-invalid' : '' }}" type="text" name="sa_id" id="sa_id" value="{{ old('sa_id', $user->sa_id) }}" required>
                @if ($errors->has('sa_id'))
                <div class="invalid-feedback">
                    {{ $errors->first('sa_id') }}
                </div>
            ...
            <div class="form-group">
                ...
                <select class="form-select {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender" required>
                    ...
                    <option value="{{ $key }}" {{ old('gender', $user->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('profile_images') ? 'is-invalid' : '' }}" type="file" name="profile_images" id="profile_images" value="{{ old('profile_images', $user->profile_images) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('client_code') ? 'is-invalid' : '' }}" type="text" name="client_code" id="client_code" value="{{ old('client_code', $user->client_code) }}" required>
            ...
            <div class="form-group">
                <label for="manager_id" class="required">{{ trans('cruds.user.fields.manager') }}</label>
                <select class="form-select select2 {{ $errors->has('manager') ? 'is-invalid' : '' }}" name="manager_id" id="manager_id" required>
                    ...
                    <option value="{{ $id }}" {{ (old('manager_id') ? old('manager_id') : $user->manager->id ?? '') == $id ? 'selected' : '' }}>
                        {{ $entry }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <div class="row" id="inputFormRow">
                    ...
                    <div class="col-lg-6">
                        <div class="input-group mb-3">
                            <input type="email" name="a_email[]" data-aname-target="#aname" id="aemail" class="form-control m-input adjustment-target-email" placeholder="Enter Email" autocomplete="off">
                ...
                @else
                @foreach ($alternative_users as $key => $a_user)
                <div class="row" id="inputFormRow">
                    <div class="col-lg-6">
                        <div class="input-group mb-3">
                            <input type="text" data-aemail-target="#aemail" name="a_name[]" id="aname" class="form-control m-input adjustment-target" placeholder="Enter Name" autocomplete="off" value="{{ $a_user->a_name }}" required>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="input-group mb-3">
                            <input type="email" name="a_email[]" data-aname-target="#aname" id="aemail" class="form-control m-input adjustment-target-email" placeholder="Enter Email" autocomplete="off" value="{{ $a_user->a_email }}" required>
                            <div class="input-group-append">
                                @if ($key == 0)
                                <button id="addRow" type="button" class="btn btn-secondary">Add
                                    Row</button>
                                @endif
                                @if ($key >= 1)
